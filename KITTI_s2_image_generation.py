...
from libs.utils.utils import merge_sweep_config, read_and_scale_bboxes_KITTI, get_text_prompts_generation
...
def DriveGEN_generate(scale, ddim_steps, sd_version,
    ...
    ood_type = 'snow'
    proto_path = "temp_data_{}/KITTI_protos".format(sd_version)
    latent_path = "temp_data_{}/KITTI_latents".format(sd_version)    
    save_path = "temp_data_{}/KITTI_res".format(sd_version)
    ...
    idxes = ['000300', '000400','000500']
    for idx in idxes:
        if os.path.exists(os.path.join(save_path, f"{idx}.png")): continue
        condition_image_path = os.path.join(args.data_root, 'image_2', idx + '.png')
        label_path = os.path.join(args.data_root, 'label_2', idx + '.txt')
        prompt, paired_objs, inversion_prompt = get_text_prompts_generation(label_path, ood_type)
        ...
        bboxes_dict = read_and_scale_bboxes_KITTI(label_path, condition_image, base_config['sd_config']['W'], base_config['sd_config']['H']) 
        ...
        gradio_update_parameter = {
            ...
            'sd_config--prototype_path': [os.path.join(proto_path, idx + '.pt')],
        ...
        resized_img.save(os.path.join(save_path, f"{idx}.png"))
