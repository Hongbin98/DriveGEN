...
from .module import prep_unet_conv, prep_unet_attention, get_self_attn_feat_KITTI
...
def create_peak_function(size, center, sigma=0.5):
    """
    创建一个二维的山峰状函数，中心值最大，向四周递减。
    
    参数：
    - size: (h, w) 表示生成函数的大小
    - center: (cx, cy) 表示山峰的中心位置
    - sigma: 控制山峰的宽度（标准差）
...
def get_query_mask(H, W, bboxes, query_num, sigma = 0.1):
    ...
    # Compute new height and width based on aspect ratio and total number of elements
    new_h = int(np.sqrt(query_num * aspect_ratio))
    new_w = int(query_num / new_h)
        
    # 检查 new_h 和 new_w 的乘积是否等于 query_num
    test_idx = 0
    while new_h * new_w != query_num and test_idx < 5:
        if new_h * new_w > query_num:
            ...
            # 如果乘积太小，增加高度
            new_h += 1
        new_w = int(query_num / new_h)  # 根据新的高度调整宽度
        test_idx += 1
    ...
    # Creating a new mask for the resized dimensions
    new_layout_mask = torch.zeros((new_h, new_w))

    for bbox in bboxes:
        xmin, ymin, xmax, ymax = bbox  # Unpack the bounding box coordinates

        # Scale the bounding box to new dimensions
        ...
        if xmin < xmax and ymin < ymax:
            ...
            if new_layout_mask[ymin:ymax, xmin:xmax].shape == torch.tensor(peak_mask).shape:
                # 将山峰掩膜叠加到layout_mask对应的位置
                new_layout_mask[ymin:ymax, xmin:xmax] += torch.tensor(peak_mask)
            else:
                print(f"Shape mismatch: layout mask {new_layout_mask[ymin:ymax, xmin:xmax].shape}, peak mask {torch.tensor(peak_mask).shape}")
                print('new_box', new_box, 'new_h_w', new_h, new_w)
        else:
            print("skipping box {},".format(bbox))
    
    # new_layout_mask = new_layout_mask > 0

    # 对生成的 layout mask 进行 min-max scaling
    ...
    non_zero_count = (new_layout_mask > 0).sum().item()
    # print(f"Non-zero elements in mask: {non_zero_count}")
...
def downsample_bboxes(bboxes, downsample_factor=8):
    return [[int(xmin / downsample_factor), int(ymin / downsample_factor), int(xmax / downsample_factor), int(ymax / downsample_factor)] for xmin, ymin, xmax, ymax in bboxes]


def replace_latents_with_data_samples(latents, data_samples_latent, car_bboxes, ped_bboxes, cyc_bboxes):
    # 将所有类别的 bboxes 合并
    all_bboxes = car_bboxes + ped_bboxes + cyc_bboxes

    # 遍历每个边界框中的每个 box 并进行替换
    for box in all_bboxes:
        x_min, y_min, x_max, y_max = box  # 获取 box 的坐标
        # 使用 data_samples_latent 中的对应区域替换 latents 中的值
        latents[:, :, y_min:y_max, x_min:x_max] = data_samples_latent[:, :, y_min:y_max, x_min:x_max]

    return latents


import torch.nn.functional as F
def compute_bbox_mse_loss(ori_img_latents, latents, bboxes, expansion_factor=0.):
    """
    计算两个 latents 在指定 bbox 区域的 MSE loss，并扩大 bbox 区域

    参数:
    - ori_img_latents: 原始图像的 latents, 维度 [bs, channel, h, w]
    - latents: 目标 latents, 维度 [bs, channel, h, w]
    - bboxes: 下采样后的 bbox 列表，格式 [[xmin, ymin, xmax, ymax], ...]
    - expansion_factor: 扩展边界框的比例 (默认为 10%)

    返回:
    - MSE loss
    """
    bs, _, h, w = ori_img_latents.size()  # 批次大小和图像尺寸
    total_loss = 0
    ...
    for i in range(1):  # 默认处理第一个批次的图像
        for bbox in bboxes:
            ...
            # 计算扩展 bbox 的面积
            expanded_area = (xmax_expanded - xmin_expanded) * (ymax_expanded - ymin_expanded)

            # if expanded_area == 0:
            #     print('area is 0! Please check this bbox')
            #     print(bbox)
            #     print(xmin_expanded, ymin_expanded, xmax_expanded, ymax_expanded)

            # 提取下采样 bbox 扩展区域的 tensor
            ...
            # 根据扩展 bbox 面积与总图像面积的比值计算权重
            weight = total_area / (expanded_area + 1e-5) if expanded_area > 0 else 1.0
            weight = weight if weight < 20 else 20

            # 将加权后的 loss 加入 total_loss
            total_loss += weight * loss

    return total_loss  # 返回总加权损失


def replace_latents_with_samples(latents, data_samples_latent, bboxes, expansion_factor=0.5):
    """
    根据传入的 bboxes，用 data_samples_latent 替换 latents 对应区域的值

    参数:
    - latents: 原始 latent 张量, 维度 [bs, channel, h, w]
    - data_samples_latent: 替换的 latent 张量, 维度 [bs, channel, h, w]
    - bboxes: 下采样后的 bbox 列表，格式 [[xmin, ymin, xmax, ymax], ...]
    ...
    for i in range(bs):
        for bbox in bboxes:
            ...
            # 替换 latents 中扩展后的 bbox 区域的值为 data_samples_latent 对应的值
            latents[i, :, ymin_expanded:ymax_expanded, xmin_expanded:xmax_expanded] = data_samples_latent[i, :, ymin_expanded:ymax_expanded, xmin_expanded:xmax_expanded]
...
class SDXLPipeline(StableDiffusionXLPipeline):
    ...
    def __call__(
        ...
        car_bboxes = downsample_bboxes(bboxes_dict['Car'])
        ped_bboxes = downsample_bboxes(bboxes_dict['Pedestrian'])
        cyc_bboxes = downsample_bboxes(bboxes_dict['Cyclist'])        
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if init_inverted_latents:
                    ...
                    latents = replace_latents_with_data_samples(latents, data_samples_latent, car_bboxes, ped_bboxes, cyc_bboxes)
                    init_inverted_latents = False
                ...
                # #### calcualte latent self-distillation loss
                # car_dis_loss = compute_bbox_mse_loss(data_samples_latent, latent_model_input[cond_control_ids], car_bboxes, expansion_factor=0.1)
                # ped_dis_loss = compute_bbox_mse_loss(data_samples_latent, latent_model_input[cond_control_ids], ped_bboxes, expansion_factor=0.1)
                # cyc_dis_loss = compute_bbox_mse_loss(data_samples_latent, latent_model_input[cond_control_ids], cyc_bboxes, expansion_factor=0.1)
                # loss = loss + (car_dis_loss + cyc_dis_loss + ped_dis_loss) * 1
                # # print(pca_loss.item(), car_dis_loss.item(), ped_dis_loss.item(), cyc_dis_loss.item())
                # print(pca_loss, car_dis_loss, ped_dis_loss, cyc_dis_loss)
    ...
    def _compute_feat_loss(self, feat, pca_info, cond_control_ids, cond_example_ids, cond_appearance_ids, step,
        ...
        if self.guidance_config.pca_guidance.structure_guidance.mask_tr > 0:
            ...
            if self.input_config.data.inversion.policy == 'share':
                ...
                #### get the mask
                H, W = 368, 1240
                car_bboxes, ped_bboxes, cyc_bboxes = self.bboxes_dict['Car'], self.bboxes_dict['Pedestrian'], self.bboxes_dict['Cyclist']

                car_layout_ref_mask = get_query_mask(H, W, car_bboxes, ca_prob_list[0].shape[1], sigma=0.1)
                ped_layout_ref_mask = get_query_mask(H, W, ped_bboxes, ca_prob_list[0].shape[1], sigma=0.1)
                cyc_layout_ref_mask = get_query_mask(H, W, cyc_bboxes, ca_prob_list[0].shape[1], sigma=0.1)

                car_layout_ref_mask = car_layout_ref_mask.flatten().to(self.running_device).unsqueeze(-1).repeat(num_control_samples, 1, ref_feat.shape[-1])
                ped_layout_ref_mask = ped_layout_ref_mask.flatten().to(self.running_device).unsqueeze(-1).repeat(num_control_samples, 1, ref_feat.shape[-1])
                cyc_layout_ref_mask = cyc_layout_ref_mask.flatten().to(self.running_device).unsqueeze(-1).repeat(num_control_samples, 1, ref_feat.shape[-1])

                ref_mask = sum(ca_prob_list) / 2 + cyc_layout_ref_mask + car_layout_ref_mask + ped_layout_ref_mask
                ref_mask = ref_mask > self.guidance_config.pca_guidance.structure_guidance.mask_tr
    ...
    @torch.no_grad()
    def sample_semantic_bases_with_realimages(self,
                              prompt: Union[str, List[str]] = None,
                              prompt_2: Optional[Union[str, List[str]]] = None,
                              height: Optional[int] = None,
                              width: Optional[int] = None,
        ...
        with self.progress_bar(total=pbar_steps * num_batch) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for latent_id, latents in enumerate(latent_list):
                    ...
                    hidden_state_dict, query_dict, key_dict, value_dict = get_self_attn_feat_KITTI(self.unet,
    ...
    def ddim_inverted_2_image(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Only require grad when need to compute the gradient for guidance
                # if require_grad_flag:
                latent_model_input.requires_grad_(True)  #### 一直都要算grad
                if require_grad_flag:
                    # predict the noise residual
                    noise_pred = self.unet(
                        latent_model_input,
